...
private:
    ...
    void EmitPhotons(const Scene& scene, int num_photons) {
        ...
        std::uniform_int_distribution<> light_dist(0, light_components.size() - 1);
        std::uniform_real_distribution<float> uniform_dist(0.0f, 1.0f);
        std::uniform_real_distribution<float> angle_dist(0.0f, 2.0f * 3.14159265f);

        for (int i = 0; i < num_photons; ++i) {
            // Select random light
            int light_idx = light_dist(rng);
            auto light_component = light_components[light_idx];
    ...
    void TracePhoton(const Scene& scene, const std::vector<TracingComponent*>& tracing_components,
        ...
        if (depth > 5 || glm::length(power) < 0.001f) return; // Max bounces / Russian roulette
