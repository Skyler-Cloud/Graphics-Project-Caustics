...
SceneParser::SceneParser() {
  fog_.color = glm::vec3(0.0f); // default fog color is black
  fog_.density = 0.0f;          // default is no fog
}
...
void SceneParser::ParseFog() {
  ...
  while (token != "}") {
    ...
    if (token == "color") {
      ...
      fog_.density = ReadFloat();
    } else if (token == "opacity") {
      fog_.opacity = ReadFloat();
    } else if (token != "}") {
      throw std::runtime_error("Bad fog token: " + token + "!");
    }
  }
}
